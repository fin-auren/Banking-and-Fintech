...
# Formulas for FinTech Impact on Bank Performance

## 1. Bank Performance Measure (PER)

The general formula for measuring the impact of FinTech on bank performance is:


PER_b,c,t = α + β₁ * FinTechBusinessModel_c,t + γ * X_b,c,t + δ * W_c,t + Other_b,c,t



Where:
- **PER_b,c,t** = Bank performance measure (ROE, ROA, NIM, NONIC, CTI).
- **FinTechBusinessModel_c,t** = Logarithm of the country-level fintech transaction volumes for P2P lending and balance sheet lending.
- **X_b,c,t** = Bank-specific controls (size, equity-to-assets ratio).
- **W_c,t** = Country-level controls (GDP growth, inflation, policy rate, concentration).
- **Other_b,c,t** = Bank fixed effects and error terms.
